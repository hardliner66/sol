...
@(test)
union_inline_test_same_bytes_value :: proc(t: ^testing.T) {
	Some_Type :: struct {
		a: int,
		b: f32,
	}
	...
	opaque: Opaque(size_of(Some_Type)) = make_opaque(original_value)
...
@(test)
union_inline_test_same_bytes_ptr :: proc(t: ^testing.T) {
	Some_Type :: struct {
		a: int,
		b: f32,
	}
...
@(test)
union_inline_test_opaque_with_different_types :: proc(t: ^testing.T) {
	Int_Type :: struct {
		a: int,
	}
	Float_Type :: struct {
	...
	int_opaque: Opaque(size_of(int)) = make_opaque(int_value)
	float_opaque: Opaque(size_of(f32)) = make_opaque(float_value)
	complex_opaque: Opaque(size_of(Complex_Type)) = make_opaque(complex_value)
...
@(test)
union_inline_test_opaque_alignment :: proc(t: ^testing.T) {
	...
	opaque: Opaque(size_of(Aligned_Type)) = make_opaque(aligned_value)
...
@(test)
union_inline_test_opaque_large_struct :: proc(t: ^testing.T) {
	Large_Type :: struct {
		data: [1024]u8,
	}
	...
	opaque: Opaque(size_of(Large_Type)) = make_opaque(large_value)
...
@(test)
union_inline_test_opaque_zero_sized_type :: proc(t: ^testing.T) {
	Zero_Sized_Type :: struct {
	}
	zero_value := Zero_Sized_Type{}

	opaque: Opaque(size_of(Zero_Sized_Type)) = make_opaque(zero_value)
...
@(test)
union_inline_test_opaque_nested_structs :: proc(t: ^testing.T) {
	Nested_Type :: struct {
		a: int,
		b: struct {
			c: f32,
	...
	opaque: Opaque(size_of(Nested_Type)) = make_opaque(nested_value)
...
@(test)
union_inline_test_opaque_pointer_handling :: proc(t: ^testing.T) {
	Pointer_Type :: struct {
		ptr: ^int,
	}
	value := 42
	...
	opaque: Opaque(size_of(Pointer_Type)) = make_opaque(pointer_value)
...
@(test)
union_inline_test_opaque_array_handling :: proc(t: ^testing.T) {
	Array_Type :: [5]int
	array_value := Array_Type{1, 2, 3, 4, 5}

	opaque: Opaque(size_of(Array_Type)) = make_opaque(array_value)
...
@(test)
union_inline_test_opaque_invalid_cast :: proc(t: ^testing.T) {
	Some_Type :: struct {
		a: int,
	}
	Other_Type :: struct {
	...
	opaque: Opaque(size_of(Some_Type)) = make_opaque(value)
